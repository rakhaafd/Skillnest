...
import { Star, CheckCircle, AlertCircle, MessageSquare, User, Trash2 } from 'lucide-react';
import Button from '../Components/Button';

const ReviewItem = ({ itemId, userId, isAuthenticated, onReviewSubmitted, isAdmin = false, onDeleteReview }) => {
        ...
        const fetchReviews = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    const unique = reviews.filter((r, i, arr) => arr.findIndex(x => x.id === r.id) === i);
                    setAllReviews(unique);
                    if (userId) {
                        const myReview = unique.find(review => review.reviewer_id === userId);
        ...
        <div className="mt-8 bg-white rounded-lg shadow-sm overflow-hidden">
            <div className="p-6">
                ...
                {canReview && !userReview && (
                    <div className="mb-6">
                        {!showReviewForm ? (
                            <Button text="Tulis Ulasan" variant="outline"
                                onClick={() => setShowReviewForm(true)} icon={<Star size={16} />} />
                        ) : (
                            <div className="p-5 bg-indigo-50 rounded-xl border border-indigo-100">
                                <h3 className="font-semibold text-gray-800 mb-4">Beri Ulasan untuk Item Ini</h3>
                                <div className="mb-4">
                                    <p className="text-sm text-gray-600 mb-2">Rating <span className="text-red-500">*</span></p>
                                    <div className="flex items-center gap-2">
                                        {[1, 2, 3, 4, 5].map((star) => (
                                            <button key={star} onClick={() => setSelectedRating(star)}
                                                ...
                                                <Star size={32} className={`cursor-pointer transition-colors ${
                                                    (hoverRating || selectedRating) >= star ? 'text-yellow-400 fill-current' : 'text-gray-300'
                                                }`} />
                                            </button>
                                        ))}
                                ...
                                <div className="mb-4">
                                    ...
                                    <textarea value={comment} onChange={(e) => setComment(e.target.value)}
                                        ...
                                        maxLength={500} rows={4} disabled={submitting}
                                ...
                                <div className="flex gap-2">
                                    <Button text={submitting ? 'Mengirim...' : 'Kirim Ulasan'} variant="solid"
                                        onClick={handleSubmitReview} disabled={submitting || selectedRating === 0} />
                                    <Button text="Batal" variant="outline" disabled={submitting}
                                        onClick={() => { setShowReviewForm(false); setSelectedRating(0); setComment(''); setError(null); }} />
                                </div>
                            </div>
                        )}
                    </div>
